## 1. Az alapvető adatbázis-készségek áttekintése
   ...
   Üdvözöljük. A kurzus néhány bemutatójában a Northwind minta relációs adatbázist és a Microsoft
...
select Salary from Employees where LastName like 'Fuller%'
--skaláris függvény, amely visszaadja egy személy fizetését, vagy 0-t, ha nem
found
go
...
- A képzési lekérdezések segítségével hozz létre egy tárolt eljárást, amely visszaadja a hiányzó készségeket
...
- A betanítási lekérdezések segítségével hozz létre egy tábla értékű függvényt, amely visszaadja a hiányzót
...
- Írjon egy forgatókönyvet, amely ellenőrzi, hogy egy alkalmazottnak szüksége van-e az a. által kínált készségekre
...
GYAKORLAT: Valósítson meg egy kurzort, amely megismétli az egyesült államokbeli ügyfeleket, és kiírja a számukat
...
- Tesztelje a fent tárolt eljárást különféle hibákra: programozási és logikai hibákra
...
GYAKORLAT: Adjon tranzakciós támogatást a saját képzésmenedzsment tárolt eljárásához és
tesztelje a különféle hibákat.
...
- `inserted`: az `INSERT` utasítással beszúrt rekordokat, illetve `UPDATE` trigger esetén az új
  ...
  rekurzívan hívja meg, az SQL Serveren legfeljebb 32 szintig. Ezt a funkciót a beágyazott
...
- Tranzakciós replikáció. Csak a módosított adatokat másolja, és lehet
...
1. Csatlakozzon a főkiszolgálóhoz, és hozz létre egy új Northwind adatbázist, ha nem
   léteznek. Futtassa a Northwind adatbázis létrehozási kiírásását.
2. A „Nem hajtható végre adatbázis-főként” hiba elkerülése érdekében, mert a megbízó
   ...
   futtassa a következő szkriptet a Northwind adatbázisban:
...
5. Módosítsd a rendelési tábla első USA rekordját a kiadónál egy UPDATE utasítással.
   ...
   legközelebb fut.
6. Végül törölje az előfizetést és a kiadványt. Ezt kiválasztással lehet elérni
...
7. Az előfizetőnél lévő replikált táblák az előfizetés törlésével nem törlődnek, így
   törölje kézzel a Rendelések táblát az nw_repl adatbázisból.
   GYAKORLAT: hozz létre egy push snapshot kiadványt az nw_repl táblába, amely másolja az alkalmazottakat
   az Alkalmazottak táblaból, amelynek címe Értékesítési képviselő. Ellenőrizd a helyes működést, majd törölje
...
4. A Szűrőtábla panelen egyenként adja hozzá a szűrőt a két táblahoz a WHERE másolásával.
...
15. Tesztelje a tranzakciós replikáció megfelelő működését. Frissítse az alkalmazotti azonosítót az elsőben
...
24. Ezután meg kell adnunk, hogy mely kiadói szerverek használhatják ezt a terjesztési adatbázist.
    A következő ablaktáblán törölje a SECOND kijelölését (mivel a SECOND nem kerül közzétételre), és a Hozzáadás megnyomásával,
...
30. Folytassa a tranzakciós kiadvány létrehozását a PRIM-en a szokásos módon (Rendelések tábla nélkül
    bármilyen szűrő).
...
35. Tesztelje az előfizetést. Nyiss meg egy lekérdezésszerkesztőt a PRIM-en, és frissítse a Rendelések tábla rekordját.
...
36. Törölje a kiadványt a PRM-nél és az előfizetést a HARMADIK oldalon. Törölje a replikált táblát
    HARMADIK.
    GYAKORLAT: implementálja a laza csatolási forgatókönyvet a rendelési esemény feldolgozásához tranzakciós használatával
...
42. Törölje a kiadványt és az előfizetést
...
A kiadvány
GYAKORLAT: A megoldás fejlesztéséhez kövesse az alábbi lépéseket.
...
20. Tesztelje az érkezési sorrendben történő alapértelmezett konfliktusfeloldást, amikor a két előfizető
...
23. Generáljon és tekintsen át replikációs szkripteket mindkét példányon a Parancsfájlok generálása innen lehetőség kiválasztásával
    a Replikáció csoport előugró menüjét.

24. Törölje az összes replikációs objektumot.
...
2. A másodlagos kiszolgálón futó másolási feladat a naplót egy konfigurálható célhelyre másolja (pl.
...
3. A másodlagos kiszolgálón futó visszaállítási feladat visszaállítja a biztonsági másolatot a másodlagos kiszolgálóra
   ...
   néhány perc alatt, anélkül, hogy hosszasan visszaállítaná a teljes biztonsági másolatot. Az elsődleges és a másodlagos
...
5. Állítsd mindkét mappa megosztását mindenkivel megosztásra (Tulajdonságok/Megosztás/Megosztás). Írja be a Mindenkit
...
7. A PRIM-en a Nortwind adatbázis Feladatok -> Szállítási tranzakciónaplók panelen engedélyezd a
   adatbázis a szállításhoz, és válaszd a Biztonsági mentés beállításai lehetőséget. Írja be a \\WIN-MTFQ8CJAV81\logs parancsot
   a biztonsági mentési mappa hálózati elérési útja, a helyi mappa elérési útja pedig a C:\ship\logs.

8. Válaszd a Feladat szerkesztése lehetőséget, és állítson be egy ütemezést, amely percenként futtatja a feladatot. Megjegyzés: ez a beállítás csak a
   demó célokra.

9. Térjen vissza az Adatbázis tulajdonságai panelre, és válaszd a Hozzáadás lehetőséget a Másodlagos adatbázisok részben:

10. A Másodlagos adatbázis-beállítások panelen írja be az új adatbázisnevet: nw_ship. Fogadd el a
    alapértelmezés az inicializáláshoz. A Fájlok másolása lapon írja be a C:\ship\dest címet a cél elérési útjaként. Állítsd be
...
12. Opcionálisan beállíthatja a HARMADIK kiszolgálót Monitor szerverként. A Monitor szerver a
    ...
    visszaállítás) nem hajtják végre sikeresen az előre beállított küszöbértéknél hosszabb ideig
...
- Vezessen le de facto oszlopmintákat reguláris kifejezésekként azokból az értékekből, amelyeket használhat
...
- Olyan kifejezés alapú relációk, amelyek az értéknek csak egy részére vonatkoznak, pl. ‘%Ltd.%’ -> ‘%Limited
  ...
  Indítsd el a DQC-t, és írja be a kiszolgáló nevét: WIN-MTFQ8CJAV81\PRIM.

- Az Adminisztráció->Konfiguráció->Általános beállítások panelen tekintse át a minimális pontszámot
  Javaslatok és minimális pontszám az automatikus javítási beállításokhoz. Ezeket a beállításokat a
  próba-hiba alapon

- A Tudásbázis kezelőpanelen tekintse át az előre telepített KB-kat
...
- A Térkép oldalon válaszd ki az AdventureworksDW2019 adatbázist, a DimCustomer táblát és
  a Vezetéknév mezőt, és rendelje hozzá a US_Last Name tartományhoz
...
- Végül mentse el a LastNameSource és LastNameOutput oszlopokat tartalmazó megtisztított táblát
...
- A Muehlheim nyílásban állítsd a típust Error értékre, és kézzel írja be a Mühlheimet a „Helyes” mezőbe.
  mezőbe, majd nyomja meg az Enter billentyűt

- Válaszd ki az új tudásbázist a főmenüből, majd az előugró menüben válaszd a `Domain management` lehetőséget.

- Manuálisan adjon hozzá egy születési_domain nevű új domaint egy egyszerű tartományszabállyal:
...
GYAKORLAT: Hozz létre egy KB-t a DimProduct tábla, ProductName mező segítségével, és mutassa be alkalmazását
mint fent.
...
- A nagy tábláknál gyakran korlátozott a betöltési idő, pl. éjszakánként egy bankrendszerben stb., amikor
  ...
  hüvelykujj az, hogy a tábla mérete meghaladja az adatbázis-kiszolgáló fizikai memóriáját.
...
- Bármely szerveroldali logikát, például a tárolt eljárásokat, újra meg kell valósítani
   ...
   pgAdmin, majd futtassa a pg_script.sql parancsfájlt a postgres adatbázisban. A forgatókönyv tartalma a következő
   következik.
...
- Valósítsd meg a hiányzó `list_orders()` függvényt, amely minden rendelési tételhez összegző sort ad vissza
  ...
  termékekből p csatlakozzon a rendelés részletei od p.productid=od.productid
  Join orders o on o.orderid=od.orderid
  csatlakozzon az ügyfelekhez c a c.customerid=o.customerid oldalon
...
DEMO: Tesztelje a rendelési tranzakció atomitását a Postgres-en. A PgAdmin konzolon szúrjon be egy
...
a webalkalmazásban, adjon le új rendelést, és ellenőrizd, hogy a hiba után az ügyfél egyenlege helyreállt
...
- Adjon hozzá 10 másodperces késleltetést a készlet és az egyenleg ellenőrzése után (importidő és
  idő.alvás(10))
...
- Indítsd el az alkalmazást 3 böngészőablakban, és rendeljen 2 egység Chai-t minden vásárlónak (érték:
...
GYAKORLAT: A fenti kódrészletet sablonként használva adja hozzá a tranzakciós vezérlést az order_proc paraméterhez
funkciót a Firestore-alapú python Flask alkalmazásban, és tesztelje a helyes működését a forgatókönyvben
...
- „A tranzakciókra vagy kötegelt írásokra vonatkozó biztonsági szabályokban 20 dokumentum hozzáférési korlát van érvényben
  lehívja a teljes atomműveletet a normál 10 hívási limiten felül minden egyes egyedi esetében
...
7. Felhőalapú adatbázis-technológiák
   ...
   dokumentumtár.
8. Indítson el egy privát böngészőt, és engedélyezd az előugró ablakokat
9. Aktiválja az 50 dolláros kupont az egyetemi e-mail címével, és kövesse az utasításokat
   kapott a Neptun üzenetben
10. Jelentkezz be a https://console.cloud.google.com címre
11. Indítson el egy új projektet, vagy törölje a meglévőt: IAM & Admin -> Erőforrások kezelése
...
14. GCP-adatbázisok->SQL->példány létrehozása -> PostgreSQL (engedélyeznie kell a számítási motort
    API)
15. Válasszon példányazonosítót és jelszót. Ez az adatbázis-kiszolgáló példány neve, nem a
...
6. A PgAdmin alkalmazásban csatlakozzon a felhőpéldányhoz a nyilvános IP-cím használatával, majd tekintse át és futtassa a
dump fájl pg_script.sql.
7. Módosítsd az IP-címet a python alkalmazásban nyilvános GCP IP-re, és tesztelje az alkalmazást. Ugyanúgy kell működnie
...
3. a promptnál írja be: gcloud init (a személyes google profilunkhoz hitelesítés szükséges) és
...
7. A northwind.py szkriptben módosítsd az adatbázis-kapcsolat paramétereit a nyilvános GCP IP-címre
   címet, és mentse el main.py néven. (Frissítse a követelmények.txt fájlt.)
...
MÉG TÖBB GYAKORLAT: Ossza fel a fenti táblát két táblára, amelyek az utolsó 5 rendelést és az egyenleget mutatják
...
8. Az elemzés előtt érdemes ellenőrizni a forrásadatok érvényességét és tartalmát.
   ...
   hozz létre és indítson el egy igény szerinti profilfeladatot a Gyors adatprofil -> Részletek lehetőségre kattintva.
   Ez a feladat a háttérben fut kb. 5 percig, majd jelenítse meg az oszlopot
   statisztikák és hisztogramok.
   b. Indítson el egy adatminőségi vizsgálatot egy manuálisan hozzáadott minőségi szabállyal (pl. Fuvar >=0).
...
13. Az eredmények azt mutatják, hogy az előrejelzés átlagos hibája 342, a magyarázott szórás 53%.
    ...
    következő parancsot. Hasonlóképpen azt taptáblajuk, hogy a legrosszabb előrejelzés 517,9 volt 17,0 helyett.
...
- Ha az adatkészletet nem az Egyesült Államokban hozták létre, hozz létre egy új adatkészletet a többrégióban „USA
  (több régió az Egyesült Államokban)” us_dataset néven, majd futtassa az alábbi lekérdezést. Ezt
...
- A MATCH minták tartalmazhatnak egy SHORTEST_PATH konstrukciót, amely a legrövidebb utat adja vissza
...
grafikonon, és írjon egy T-SQL szkriptet, amely átmásolja a tartalmat az új gráftáblákba. Kérdezze meg a bevételt
...
GYAKORLAT: Írjon egy rövid T-SQL szkriptet, amely egy véletlenszerű gráf adatbázist generál egyetlen csomóponttípussal
...
N=100, E=3, és növeljük az N-t, amíg a végrehajtási idő meg nem haladja a 30 másodpercet. Mit vársz és mit csinálsz
méred?
Tippek
...
- A sorszámláló üzeneteket kikapcsolhatja és a tényleges végrehajtási időt mérheti a
  ...
  tiltsa le a statisztikai idő/io beállítást az adatbázis generálása közben.
...
- RETURN: keresd meg és adja vissza a megfelelő részgráfot
...
<-[:IRÁNYÍTOTT]//figyelje meg a nyíl irányát
...
set p.birthdate = dátum({év: 2018}) //felülírja, ha létezik
visszatérés p
...
2. Importálja a minta Filmek adatbázist az adatbázis kiírásából. Ehhez használd a Megnyitás gombot
   ...
   a Neo4j böngészőben. Futtassa a szkriptet a kék háromszögre kattintva. -> „179 címke hozzáadva, létrehozva
   179 csomópont, 580 tulajdonság beállítása, 258 kapcsolat létrehozása”
3. A neo4j$ promptba írja be a CALL db.schema.visualization() parancsot a csomópont megtekintéséhez és
   kapcsolattípusok
...
- Kérdezze meg azokat a személyeket, akik olyan filmet rendeztek, amelyben szerepeltek
...
match p=shortestPath((p1:Person)-[:FOLLOWS*]-(p2:Person)) //p egy elérési út. Jegyezze meg a
*
...
2. Exportálja ezeket a nézeteket CSV-fájlokba az SSMS Adatexportálás eszközzel
3. Másolás
   a
   fájlokat
   be
...
vissza c;
GYAKORLAT: A Northwind adatok betöltése után kérdezze le a Cypherben a bevételt év és terméknév szerint.
Hasonlítsa össze a relációs verziót ugyanazon lekérdezés gráf alapú verziójával.
GYAKORLAT:

1. Állítsd le a Neo4j adatbázist

2. Írjon egy T-SQL parancsfájlt egy véletlenszerű gráf létrehozásához 1000 csomóponttal és 500 éllel az SQL Serverben
   és mérje meg az átmérő kiszámításához szükséges időt

3. Exportálja a grafikont CSV-fájlokba
...
7. Magyarázza el az eredményeket
...
- Nagyméretű videó- ​​és egyéb BLOB-fájlok esetén a FileTables technológia ajánlott az SQL Serveren.
   ...
   GYAKORLAT: Valósítson meg egy FileTable megoldást a kurzus képernyőrögzítő videóinak tárolására.
...
- Az összes tábla összesítő táblaának létrehozásával ellenőrizheti az áttelepítések számát
   blokkolók. Látható, hogy a legrosszabb eset a Vevők tábla 36 blokkolóval. Ez esedékes
...
2. Állítsd be a teszteléshez szükséges paramétereket, és futtassa le a procit 10000-szer 3 párhuzamos munkamenetben, olvasás közben
...
4. A Termékek, Rendelések, Rendelés részletei, Ügyfelek táblák áttelepítése:
   a. Hozz létre egy új adatbázist Northwind_mot néven, és futtassa benne a javított dump szkriptet
   b. Törölje az idegenkulcs-megszorításokat a fenti táblákból
...
6. Futtassa a régi sorrendű procit, tesztelje és hasonlítsa össze a futási időket
7. Készítsd el a tárolt eljárás natívan lefordított változatát, és ismételje meg a teszteket
8. Magyarázza el az eredményeket!
...
3. Az adatfájl elveszett -> készítsen biztonsági másolatot a tranzakciós naplóról, használd a korábbi teljes és differenciális biztonsági mentéseket
   ...
   állítsd le az SQL Server PRIM szolgáltatást, törölje az adatbázisfájlt, indítsd újra a szolgáltatást, és állítsd vissza innen
   ...
   készítse elő manuálisan. Alternatív megoldásként a biztonsági mentési feladatok integrálhatók egy adatbázis-karbantartási tervbe.
...
4. Hozd létre és tesztelje a riasztást
   ...
   profil panelen írja be a prim_mail parancsot a profilnévhez, majd kattints az SMTP-fiók hozzáadása lehetőségre. Add meg a mi nevét
   SMTP szerver.
...
Válaszd az Operátorok lehetőséget az SQL-kiszolgáló ügynök csomópontjából, és adjon hozzá egy új operátort. Az „E-mail név” szövegben
mezőbe írja be a saját e-mail címét.
...
A Beállítások lapon adjon hozzá egyéni üzenetet. Állítsd a késleltetést 1 percre. Ezt a beállítást csak demóhoz használjuk
...
Tesztelje a riasztást a következő szkripttel.
válaszd ki
...
a big_table, és tiltsa le vagy törölje a figyelmeztetést.
...
GYAKORLAT: hozz létre egy új szerepet a db_datareader és a db_datawriter szerepkör fúziójaként, és rendelje hozzá a
...
csatlakozzon a sys.database_principals-hoz DP2-ként a DRM-en.member_principal_id = DP2.principal_id
...
8. Ha szeretnéd látni a visszafejtett eredményeket, nyiss meg egy új kapcsolatot, és a Beállításokban válaszd a lehetőséget
   További kapcsolati paraméterek, és írja be: „Oszloptitkosítási beállítás = Engedélyezve”.
