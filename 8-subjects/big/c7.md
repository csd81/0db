...
- Data security may be compromised if highly sensitive data (like personal healthcare records) physically leave the company network. Incidents or serious data loss may also occur.[^31]
- Critical services, like production system control should not depend on Internet access. These latter two concerns can be alleviated in a **private cloud**.[^32]
...
2. Choose an instance id and password. This is the name of the database server instance, not the name of the database or the database user. Set:
   - region: Europe, PostgreSQL version 13, single zone (no failover)
   - customize your instance: public IP enabled
...
7. Change the IP in the python app to the public GCP IP and test the app. It should work just as with the local server. Check that the new orders appear in the Postgres tables.
...
## Implementing the demo app on GCP[^33] -> Cloud programming MSc course

We'll deploy the app to the GCP app engine.[^34]
...
3. At the prompt type: `gcloud init` (requires authentication to your personal Google profile) and select YOUR-PROJECT-ID.
...
10. `gcloud app deploy`
    You'll receive a target URL on which you can access the application. You can also start a browser for the app by: `gcloud app browse`
...
> **EVEN MORE PRACTICE:** Split the table above in two tables that show the last 5 orders and the balance of the customers involved separately.
...
"BigQuery is designed to query structured and semi-structured data using standard SQL."[^35]
...
-- if the select list does not contain the class label, you get the error message
-- 'Unable to identify the label column'
FROM `northwind.nw`
WHERE value_numeric is not null
```

11. Build the `northwind.nw_model` model (ca. 30 seconds), then go to the model. Check the Details and the Evaluation tabs. You can see that the data set was split automatically for a training table (1702 records) and an evaluation table (453 records). The mean absolute error is 301.

12. We can evaluate the model using the full data set (though this is not a recommended best practice in data science and we do it only for demonstrating the EVALUATE method). You can use the following command:
...
> **PRACTICE:** Use the public `bigquery-public-data.samples.natality`[^39] test table to predict the weight of a baby using linear regression. This table contains many records, so you can sample the record set with a `WHERE RAND() < 0.0001` clause.
>
> Some help:
>
...
[^31]: See: https://www.theguardian.com/australia-news/article/2024/may/09/unisuper-google-cloud-issue-account-access

[^32]: E.g. https://www.openstack.org/

[^33]: If you are already familiar with cloud programming concepts, you can skip this section and go on with the app running on localhost.

[^34]: See: https://medium.com/@dmahugh_70618/deploying-a-flask-app-to-google-app-engine-faa883b5ffab

[^35]: See: https://panoply.io/data-warehouse-guide/bigquery-architecture/

[^36]: See: https://medium.com/google-cloud/bigquery-explained-storage-overview-70cac32251fa

[^37]: See: https://cloud.google.com/bigquery-ml/docs/reference/standard-sql/bigqueryml-syntax-create

[^38]: However, this feature does not seem to work.

[^39]: See: https://cloud.google.com/bigquery-ml/docs/bigqueryml-natality
