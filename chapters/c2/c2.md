...
```
[Orders table] → [INSERT trigger] → [Event log table] → [Order processing job] → [Manufacturing tables]
```
...
- **`deleted`:** holds the records deleted from the table in case of a DELETE trigger or the original (old) records updated in case of an UPDATE trigger. An update is logically equivalent to a delete followed by an insert. The `deleted` table is empty in case of an INSERT trigger.
...
- **Automating business workflow processes.** Examples in the Northwind database:
  ...
  - We automatically update the UnitsInStock field of the Products table when the quantity field in a corresponding Order Details record changes (UPDATE trigger).
...
**PRACTICE:** Assume that in the problem above more than one Order Details record is updated at a time.
...
--the insert statement has been rolled back -> we crashed the trading system
```
...
declare @orderid int
select @orderid=orderid from inserted  --there can be more than a single record in inserted
