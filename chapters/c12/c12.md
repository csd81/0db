...
In order to check database integrity, it is a good practice to run the following command as often as a full backup is created:

```sql
DBCC CHECKDB ('DB_name') WITH NO_INFOMSGS, ALL_ERRORMSGS
```

Any output means a corruption of the database.
...
2. The server process stops (the in-memory data and log buffers are lost) → when the server restarts, it first rolls forward those transaction steps from the transaction log for which the relevant data pages were not flushed to the data files, then it rolls back all uncommitted transactions.
3. The data file is lost → back up the transaction log, use the previous full and differential backups and then the log backup to restore the database, then roll back all uncommitted transactions.

Taking regular backups is a common way to prepare for disaster recovery. Backup files are to be stored on media different from that of the database data and log files. The three most common strategies:

- **#1 — The minimum:** use full backups in SIMPLE recovery mode[^55], e.g. on a daily basis in off-load periods. The backups should be stored and re-written in a round-robin fashion, e.g. over a weekly period. In this way the data loss can be limited to one day. It is a good practice to include in the backup also the `master`, `model` and `msdb` databases as well.
...
Backups can be performed in the form of `BACKUP DATABASE` / `RESTORE DATABASE` SQL statements like this:[^56]
...
> **PRACTICE:** We simulate a disk error. Create a full backup of the Northwind database on a file device, stop the SQL Server PRIM service, delete the database file, start the service again and restore from backup, using the `WITH REPLACE` option. Check the contents.

For a regular backup, an option is to execute the `BACKUP` SQL statement in a scheduled job that we prepare manually. Alternatively, backup jobs can be integrated into a database maintenance plan.
...
> **PRACTICE:** Implement backup strategy #1 in a new maintenance plan for a new test database by setting the schedule to every 2 minutes (for demo purposes). Also include index re-computing and integrity checking as tasks to be executed on a daily basis. Use **Management → Maintenance Plans**. Do not use the wizard. We set the 2-minute recurring interval only for demo purposes. Check that the backups are generated every 2 minutes.

We cannot test the cleanup task because we cannot set a delete age less than a day.

> **PRACTICE:** Implement backup strategy #3 in a new maintenance plan for the Northwind database.
...
> **PRACTICE:** In this demo, we set up an alert that fires when the size of the Northwind database hits 150% of the current size. First we check the current size of the database in the `C:\Program Files\Microsoft SQL Server\MSSQL14.PRIM\MSSQL\DATA` folder: 8.2 MB.
...
3. Create an operator (the person who will receive the alert and handle the issue)
4. Create and test the alert

### Setting up database mail

Select **Server → Management → Database Mail → Configure Database Mail**, then in the New Profile panel, type `prim_mail` for the profile name, then click **Add SMTP account**. Enter the name of our SMTP server.

Then you must specify the name of the new mail profile that will use the SMTP account. By making the profile public, every user can use it for sending emails.
...
Select **Alerts** from the SQL Server Agent node and add a new alert. Set the response to **Notify operator (NW system administrator) by email**.

In the **Options** tab, add a custom message. Set the delay to 1 minute. We use this setting only for demo purposes.

> **NOTE:** If you set the Delay between responses to 0, the response will be repeated continuously as long as the alert condition is satisfied.
...
Database objects belong to a schema and the schema has a single owner (different from being a member of the `db_owner` database role). The owner can DROP the schema and create or drop objects in the schema. Privileges are normally controlled at the schema/role level and not at the individual user/table level.
...
In SQL Server 2017, fields can be classified automatically or manually with respect to info type (e.g. "Personal") and sensitivity level (e.g. "Confidential-GDPR"). Use the database **Tasks → Classify data** tool. You can save the accepted recommendations. We can also use the Extended Properties catalog view called `sys.extended_properties` to query this information. You can also run an automatic vulnerability analysis.
...
> **PRACTICE:** Try the `default()` and `random()` masking functions on the `AdventureworksDW2019.dbo.DimCustomer` table.
...
- Use the mmc console **All tasks → Manage private keys** to add the `MSSQL$PRIM` user as a user with full control of the private key of the certificate.
- In SQL Server Configuration Manager use **Protocols → Properties** to set the **Enforce encryption** option and restart the service.
...
--it is wise to create a backup of it for disaster recovery
...
> **DEMO:** We add Always Encrypted to the Address column of the Employees table.
>
> 1. Select **Northwind → Tables → Employees → Encrypt Columns** and select the Address field.
> 2. Choose **Randomized** type. This means that grouping, filter by equality, and joining tables on encrypted columns will not be possible on the server, but an ECB[^59]-type attack against the column is prevented.
> 3. Choose to generate a new column key.
...
> 7. Run a `SELECT *` query on the table to see what the administrator sees (the encrypted values).
...
> **PRACTICE:** Select the **Deterministic** encryption type for the Title column and verify that the same ciphertext will be generated for the same plaintext, making an ECB attack possible.

---

[^55]: The SIMPLE recovery mode means that the inactive parts of the log are regularly truncated.

[^56]: SQL scripts can be generated from each dialog panel of SSMS.
