...
The columnstore is an alternative way of storing relational data. Recommended for **large data warehouse tables with many fields and infrequent changes**. In the table blocks (pages), the fields for a record are not stored together; instead, each field is stored separately, and the values for a record are linked together.[^16] It owes its speed to the few I/O operations (block reads) that result from the following factors:

- The compression of the pages[^17] (reduced size → fewer pages), as well as the fact that queries usually do not require reading the whole record, only a small number of fields
- This is particularly apparent when the table has a very large number of fields — which is typical for data warehouses.
...
- **Page compression**: compresses whole pages and is more applicable to data warehouse applications or to OLTP applications with infrequent updates. Compression is particularly effective when the values of a field are quite similar — these are called *low cardinality* fields (i.e. fields with few distinct values).
...
- **Logical reads** are 8 KB page reads from the data buffer
- **Physical reads** are reads that had to be fetched from the storage (from the OS) — these block the execution of the query
...
2. Load existing data (initial load)
...
The Postgres alternative needs the direct manipulation of the partitions as tables.[^18]
...
--for loading, the new partition can be created independently, then attached to the
--parent table
...
In Postgres, you can also use the table inheritance mechanism to support partitioning, however, the process is more complicated:[^19]
...
[^16]: See: https://learn.microsoft.com/en-us/sql/relational-databases/indexes/columnstore-indexes-overview?view=sql-server-ver16

[^17]: See: https://learn.microsoft.com/en-us/sql/relational-databases/data-compression/data-compression?view=sql-server-ver16

[^18]: See: https://www.postgresql.org/docs/current/ddl-partitioning.html#DDL-PARTITIONING-DECLARATIVE-BEST-PRACTICES

[^19]: See: https://medium.com/timescale/scaling-partitioning-data-postgresql-10-explained-cd48a712a9a1
