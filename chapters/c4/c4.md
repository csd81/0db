...
SalesReasonReasonType  --3 groups with 4, 5 and 1 members
...
You can download this database from https://github.com/Microsoft/sql-server-samples/releases/download/adventureworks/AdventureWorksDW2019.bak
...
- **DQS_MAIN:** stored procedures that implement the DQS engine, plus Knowledge Bases, e.g. US – Last Name knowledge base, reference data: e.g. cities in Hungary. Roles in the DQS_MAIN database: `dqs_administrator`, `dqs_kb_editor`, `dqs_kb_operator` (can be assigned to different users)
- **DQS_PROJECTS:** data needed for cleaning and matching projects
- **DQS_STAGING_DATA:** temporary storage for data to be cleaned, and the results of cleaning
...
**Composite domains** can be created of semantically connected fields, e.g. city, street, etc. parts of an address stored in more than one column.

We have some default KBs already installed in the DQS_MAIN on the PRIM server like Country/Region, US-Counties, US-Last names, etc. We can also develop our own KBs.
...
- Start the cleaning process. Since we have more than 18,000 last names in the DimCustomer table to cleanse, pre-processing will run for ca. 5 minutes, cleansing for ca. 30 s. **WARNING:**[^15]
...
- Select Knowledge discovery, set the source to the `vi_places` view, and create two domains called `cities_domain` and `countryregion_domain` using the fields of the same name of the `vi_places` view.
...
- Manually add a new domain called `birth_domain` with a simple domain rule:
  ...
  **Build a Rule: young_enough**
  ...
  - `birth_domain` → Value is greater than: Thursday, January 1, 1920
