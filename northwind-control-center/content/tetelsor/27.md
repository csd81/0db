...
- **Csillag-rajz** (**Alfred Petersen**, 1849–1922, dán matematikus): külső 5-szög ($A_1, \dots, A_5$), belső 5-szög-pentagram ($B_1, \dots, B_5$), $A_i$ és $B_i$ minden $i$-re összekötve.
- **„Kempe-féle" rajz** (**Julius Kempe**, 1839–1910, dán): a 10 csúcs a $\binom{5}{2} = 10$ db kétpontú részhalmazaként $\{1,\dots,5\}$ fölött, két csúcs szomszédos $\iff$ a megfelelő részhalmazok **diszjunktak**.
