...
| Típus | $G$ alak | $[A]_{ij}$ definíció | Értékkészlet |
|---|---|---|---|
| **1.** Egyszerű | hurok+többszörös tilos | $1$ ha él, $0$ ha nincs | $\{0,1\}^{n \times n}$ |
| **2.** Multi | hurok+többszörös OK | $k$ (mult.) ha nem hurok, **$2k$** ha hurok | $\mathbb{N}^{n \times n}$ |
| **3.** Súlyozott | $w: E \to \mathbb{R}_+$ | $w(v_i, v_j)$ ha él, **$+\infty$** ha nincs | $\mathbb{R}_+^{n \times n}$ |
| **4.** Irányított egyszerű | $(v_i, v_j) \neq (v_j, v_i)$ | $+1$ ha $(v_i, v_j) \in E$, $-1$ ha $(v_j, v_i) \in E$, 0 egyébként | $\{-1, 0, +1\}^{n \times n}$ |

A főtételek érvényessége típusonként különbözik: az 1. + 2. típusra a fokszám-összeg formulák, a kézfogás, hatványos séta-számolás mind igaz; a 3. típuson a hatványnak más jelentése van; a 4. típuson $A^T = -A$ (ferdén szimmetrikus).
