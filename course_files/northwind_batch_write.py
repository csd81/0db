    ...
    try:
        if request.method == 'POST': # we take the order only if the route was called form the other form
            ...
            if (success==0): # we modify the db only if everything is fine
                ...
                batch.update(doc_product, {'unitsinstock': stock-float(request.form['qt'])}) # this update will be rolled back
# this is where we make an error
                print(1/0)
                logging.info("...updating balance...")            
                batch.update(doc_customer, {'balance': balance-unitprice*float(request.form['qt'])}) 
                logging.info("...adding new order...")            
                doc_order = db.collection('orders').document()   # a new document is created, automatic ID is generated
                batch.set(doc_order, {'customerid': request.form['customer'], 'orderdate': datetime.now(), 'customer' : 'customers/'+request.form['customer']+'/'}) # the customer field is a reference type
                item = doc_order.collection('details').document()
                batch.set(item, {'productid': request.form['product'], 'product': 'products/'+request.form['product']+'/', 'quantity': request.form['qt']})
                logging.info("...commiting batch...")  
                batch.commit()
